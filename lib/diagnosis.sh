...
diagnosis_run() {
  ...
  if [ -n "$WIFI_SNR" ] && [ "$WIFI_SNR" -lt 20 ]; then
    add_diag warn "Other electronics or nearby WiFi networks are interfering with yours (signal-to-noise ratio ${WIFI_SNR} dB — below 20 dB is noisy). Try switching to a less crowded channel in your router settings."
  fi
  if [ -n "$GW_LOSS" ] && [ "${GW_LOSS%.*}" -ge 20 ]; then
    if [ "$IS_WIFI" -eq 1 ] && [ -n "$WIFI_RSSI" ] && [ "$WIFI_RSSI" -lt -70 ]; then
      add_diag critical "You're losing packets between your Mac and your router, and your WiFi signal is weak — the WiFi link is the bottleneck, not the router or the ISP. Move closer to the router or switch WiFi channel."
    else
      add_diag critical "You're losing packets between your Mac and your router even though the WiFi signal is strong — the router itself is misbehaving. Try rebooting it (unplug for 30 seconds, plug back in)."
    fi
  fi
  ...
  if [ "$PUBLIC_OK" -eq 0 ] && [ -n "$GW_LOSS" ] && [ "${GW_LOSS%.*}" -eq 0 ]; then
    if [ "$DNS_OK" -eq 0 ]; then
      add_diag critical "Your local network is working but the wider internet is unreachable, and name lookups are also failing — likely a DNS or upstream-ISP outage. Try opening http://1.1.1.1 in a browser: if it loads, the problem is DNS; if not, it's the ISP."
    else
      add_diag critical "Your local network is healthy and name lookups work, but no public website responds — almost certainly an outage on your ISP's side. Check their status page or call support."
    fi
  fi
  ...
  case "${BUFFERBLOAT_GW_GRADE:-}" in
    C)   add_diag warn     "Your router gets a bit sluggish under load — when something is downloading or uploading heavily, calls and games will feel laggy (extra +${BUFFERBLOAT_GW_DELTA} ms delay, bufferbloat grade C). Fix: enable \"Smart Queue Management\" or \"QoS\" in your router's admin page." ;;
    D|F) add_diag critical "Your router chokes under load — whenever someone's downloading or uploading, Zoom / FaceTime / WhatsApp calls will glitch and games will lag badly (extra +${BUFFERBLOAT_GW_DELTA} ms delay, bufferbloat grade ${BUFFERBLOAT_GW_GRADE}). Fix: enable \"Smart Queue Management\" or \"QoS\" in your router's admin page, or replace the router with one that supports it." ;;
  esac
  case "${BUFFERBLOAT_INET_GRADE:-}" in
    C)   add_diag warn     "The bottleneck under heavy use is your ISP's equipment, not your router (+${BUFFERBLOAT_INET_DELTA} ms extra delay under load, bufferbloat grade C). Try a modem firmware update if you control it; otherwise this is the ISP's responsibility." ;;
    D|F) add_diag critical "Your ISP's equipment is the bottleneck — under load your connection adds +${BUFFERBLOAT_INET_DELTA} ms of delay (bufferbloat grade ${BUFFERBLOAT_INET_GRADE}), enough to ruin voice/video calls and multiplayer games. Call your ISP and ask about firmware updates or a plan with better latency." ;;
  esac

  # M1 — path MTU below 1500.
  if [ -n "$MTU_EFFECTIVE" ] && [ "$MTU_EFFECTIVE" -lt 1400 ]; then
    add_diag critical "Most websites won't load fully — your network is silently dropping anything bigger than ${MTU_EFFECTIVE} bytes per packet. Cause is usually a VPN or DSL link that hasn't been configured to tell other devices about the smaller size. Disconnect any VPN; if it persists, check your router's WAN settings (technical: path MTU ${MTU_EFFECTIVE} — needs MSS clamping)."
  elif [ -n "$MTU_EFFECTIVE" ] && [ "$MTU_EFFECTIVE" -lt 1500 ]; then
    add_diag warn "Some websites load fine and others hang forever loading — your network is silently dropping packets above ${MTU_EFFECTIVE} bytes. Usually caused by a VPN, a tunneled connection, or a DSL link. Try disconnecting any VPN; if it persists, ask your ISP or check your router's WAN-MTU / MSS-clamping setting."
  fi

  # MT1 — first lossy hop.
  if [ -n "$MTR_FIRST_LOSSY_HOP" ]; then
    add_diag warn "Packet loss starts at $MTR_FIRST_LOSSY_HOP — that hop on the path to the internet is dropping packets. Hops after it usually inherit the problem rather than adding their own. Whoever owns that hop (your router, your ISP, or a transit network) needs to look at it."
  fi
  ...
  if [ -n "$NTP_DRIFT_S" ] && awk -v d="$NTP_DRIFT_S" 'BEGIN{exit !((d<0?-d:d) > 30)}'; then
    add_diag critical "Your Mac's clock is off by ${NTP_DRIFT_S} seconds — every secure website (anything starting with https://) will refuse to connect because clock-based certificate checks will fail. Fix: open System Settings → General → Date & Time and turn \"Set date and time automatically\" on."
  elif [ -n "$NTP_DRIFT_S" ] && awk -v d="$NTP_DRIFT_S" 'BEGIN{exit !((d<0?-d:d) > 1)}'; then
    add_diag warn "Your Mac's clock is off by ${NTP_DRIFT_S} seconds. Most apps will be fine but some authenticated services (banking apps, work VPNs) may refuse to connect. Turn on \"Set date and time automatically\" in System Settings → General → Date & Time if this persists."
  fi

  # DI-1 — duplicate IP / incomplete gateway ARP.
  if [ "$ARP_GW_INCOMPLETE" -eq 1 ]; then
    add_diag critical "Your Mac can't find your router on the local network — the connection between them is broken at the hardware layer. Check the ethernet cable, the WiFi connection, or that the right router is set as the default. Nothing else above this matters until this is fixed."
  fi
  if [ -n "${ARP_DUPLICATE_IPS//[[:space:]]/}" ]; then
    add_diag critical "Another device on your network is using the same IP address as one of these: ${ARP_DUPLICATE_IPS}. Both devices will randomly steal each other's traffic. Cause is usually a manually-set IP that collides with one the router handed out, or a second router on the network. Find and fix the duplicate."
  fi

  # DH-1 — DHCP lease expires soon.
  if [ -n "$DHCP_TIME_REMAINING_S" ] && [ "$DHCP_TIME_REMAINING_S" -gt 0 ] && [ "$DHCP_TIME_REMAINING_S" -lt 3600 ]; then
    add_diag warn "Your Mac's network-address lease from the router expires in $((DHCP_TIME_REMAINING_S / 60)) minutes. Normally it renews automatically, but if your router is rebooting or out of addresses at that moment, you'll suddenly lose the network with no warning. Keep an eye out."
  fi

  # DH-2 — DHCP vs system DNS mismatch.
  if [ -n "$DHCP_DNS_SERVERS" ] && [ -n "$SYS_RES" ] && ! printf '%s' "$DHCP_DNS_SERVERS" | grep -qF "$SYS_RES"; then
    add_diag info "Your router suggested $DHCP_DNS_SERVERS for name lookups, but your Mac is using $SYS_RES instead — somebody manually overrode it. Fine if you did it on purpose (1.1.1.1 and 8.8.8.8 are common choices); surprising if you didn't."
  fi
  ...
  for s in critical warn info; do
    for i in "${!DIAG[@]}"; do
      if [ "${DIAG_SEV[$i]}" = "$s" ]; then
        ...
        [ "$first" -eq 0 ] && say ""
        first=0
        _print_diagnosis_paragraph "$s" "${DIAG[$i]}"
