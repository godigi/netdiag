...
| `wifi` | object | SSID, BSSID, security; `rssi`/`noise`/`snr`/`channel`/`phy`/`tx_rate` are `null` without `sudo`. `privileged` says whether that sudo-only scrape actually ran — without it, a `null` radio field is ambiguous between "measured and quiet" and "never measured", which is what makes a stored flapping episode undiagnosable after the fact. `name_hidden` is `true` when macOS withheld the SSID from an unprivileged caller, so `ssid` is a placeholder rather than a name (see `DIAGNOSIS-RULES.md#wi-1--macos-is-withholding-the-networks-name`). |
